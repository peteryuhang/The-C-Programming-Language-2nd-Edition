...
/* 
 * Our getch and ungetch do not handle a push-back EOF correctly.
 * Decide what their properties ought to be if an EOF is pushed back,
 * then implement your design
 * 
 * Answer: The EOF is -1, which couldn't been handled by current getch and ungetch
 * because the buf array type is char. Can change the datatype and make it able to
 * handle EOF
 */
int main() {
  int type;
